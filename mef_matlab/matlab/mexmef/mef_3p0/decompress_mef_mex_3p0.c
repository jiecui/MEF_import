...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if(nrhs < 1) {
        mexErrMsgIdAndTxt( "MATLAB:decompress_mef_mex_3p0:noChannelPathArg", "channelPath input argument not set");
    } else {
        if(!mxIsChar(prhs[0])) {
            mexErrMsgIdAndTxt( "MATLAB:decompress_mef_mex_3p0:invalidChannelPathArg", "channelPath input argument invalid; should be type string (array of characters)");
        }
        if(mxIsEmpty(prhs[0])) {
            mexErrMsgIdAndTxt( "MATLAB:decompress_mef_mex_3p0:invalidChannelPathArg", "channelPath input argument invalid; argument is empty");
        }
    }
    // set the path
    ...
    // note: if the password passed to any of the meflib read function is an empty string,
    // than 'process_password_data' function in 'meflib.c' will crash everything, so make
    // sure it is either NULL or a string with at least one character
    if (nrhs > 1) {
        if (!mxIsEmpty(prhs[1])) {
            if (!mxIsChar(prhs[1])) {
                mexErrMsgIdAndTxt( "MATLAB:decompress_mef_mex_3p0:invalidPasswordArg", "password input argument invalid; should be string (array of characters)");
            }
            // set the password
    ...
    if (nrhs > 2) {
        ...
        if (!mxIsChar(prhs[2])) {
            mexErrMsgIdAndTxt( "MATLAB:decompress_mef_mex_3p0:invalidRangeTypeArg", "rangeType input argument invalid; should be string (array of characters)");
        }
        char *mat_range_type = mxArrayToString(prhs[2]);
        for(int i = 0; mat_range_type[i]; i++)
            mat_range_type[i] = tolower(mat_range_type[i]);
        if (strcmp(mat_range_type, "time") != 0 && strcmp(mat_range_type, "samples") != 0) {
            mexErrMsgIdAndTxt( "MATLAB:decompress_mef_mex_3p0:invalidRangeTypeArg", "rangeTtype input argument invalid; allowed values are 'time' or 'samples'");
        }
        ...
        if (nrhs > 3) {
            ...
            if (!mxIsNumeric(prhs[3]) || mxGetNumberOfElements(prhs[3]) > 1) {
                mexErrMsgIdAndTxt( "MATLAB:decompress_mef_mex_3p0:invalidRangeStartArg", "rangeStart input argument invalid; should be a single value numeric (either -1 or >=0)");
            }
        ...
        if (nrhs > 4) {
            ...
            if (!mxIsNumeric(prhs[4]) || mxGetNumberOfElements(prhs[4]) > 1) {
                mexErrMsgIdAndTxt( "MATLAB:decompress_mef_mex_3p0:invalidRangeEndArg", "rangeEnd input argument invalid; should be a single value numeric (either -1 or >=0)");
            }
